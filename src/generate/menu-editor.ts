...
export function buildInlineMenuEditBarHtml(): string {
  ...
  <button type="button" class="btn" id="btn-send-draft">Enviar borrador</button>
</div>`;
}
...
export function buildInlineMenuEditScript(
  ...
  const waDraftMessage = [
    `Hola — soy de ${name}.`,
    "",
    `Te mando el JSON de mi menú (slug: ${slug}).`,
    `Adjunto el archivo menu-${slug}.json que descargué del menú.`,
    "",
...
(function () {
  ...
  function readDom() {
    list.querySelectorAll('.cat').forEach(function (sec) {
      ...
      var h = sec.querySelector('h2');
      if (h) {
        state.categories[ci].label = (h.textContent || '').trim() || state.categories[ci].label;
      ...
      sec.querySelectorAll('.item').forEach(function (row) {
        ...
        row.querySelectorAll('input[data-f]').forEach(function (inp) {
          state.categories[ci].items[ii][inp.getAttribute('data-f')] = inp.value;
        });
      });
    });
  }
  ...
  list.addEventListener('blur', function (ev) {
    if (ev.target && ev.target.getAttribute && ev.target.getAttribute('data-f') === 'label') {
      readDom();
      saveLocal();
      render();
    }
  ...
  if (sendBtn) sendBtn.addEventListener('click', function () {
    ...
    saveLocal();
    downloadJson(draft);
    setStatus('JSON descargado — adjúntalo en WhatsApp.');
    var href = WA_HREF;
    if (!href && WA_DIGITS) {
      var msg = 'Hola — soy de ' + NAME + '.\\n\\nTe mando el JSON de mi menú (slug: ' + SLUG + ').\\nAdjunto menu-' + SLUG + '.json.\\n\\n¿Me lo publican?';
